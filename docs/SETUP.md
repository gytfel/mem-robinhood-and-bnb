...
```
/dry              погонять стратегию на бумаге, деньги не тратятся
/set gasmode turbo   поднять газ ×2.5, когда нужно успеть первым
/set route v3        торговать только через Uniswap V3
/set maxloss 3       стоп после трёх убытков подряд, снимается командой /on
/set dayloss 0.5     остановиться, потеряв 0.5 монеты за сутки
/stats 24            посмотреть, какие фильтры отсеивают больше всего
/pnl 30              отчёт за месяц с файлом CSV
```
