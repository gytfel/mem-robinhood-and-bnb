...
| `sniper doctor` | проверяет ключи, Telegram, каждый RPC, площадки и поддержку симуляции |
| `sniper run` | запускает бота (`--log-level DEBUG` — подробные логи) |
| `sniper check 0xТокен` | полная проверка токена прямо в терминале |
| `sniper discover 0xРоутер` | определяет версию DEX, достаёт фабрику и WETH; `--quoter` и `--token` проверяют адреса котировкой |
...
Все настройки видны командой `/config`, меняются `/set имя значение`
(или кнопками в `/settings`). Значения хранятся отдельно для каждой сети,
кроме помеченных «общая».
...
```
# осторожный режим: меньше сделок, выше доля прибыльных
/set ladder 80:50,250:30     половину фиксируем на +80%, треть на +250%
/set breakeven 40            после +40% сделка больше не может стать убыточной
/set rugguard 40             выходим, если из пула вынули 40% ликвидности
/set ownershare 10           у владельца не больше 10% предложения
/set minedge 50              издержки должны оставлять минимум +50% запаса
/set deadtime 45             через 45 минут закрываем то, что не выросло

# быстрый режим: успеть в первые блоки
/set gasmode turbo           газ ×2.5
/set exitgas 2               выход ×2 к обычному газу
/set exitslip 40             на выходе допускаем 40% проскальзывания
/set preapprove on           approve заранее, продажа без лишней транзакции

# сначала на бумаге
/dry                         тестовый режим
/pnl test 7                  что получилось
/optimize test               какие TP/SL были бы лучше
```
...
| Токены находятся редко | `/stats` покажет, сколько пулов найдено, сколько ждут ликвидности и какие фильтры их отсеивают. Медленная проверка (>10 с на токен) — признак медленного RPC |
