...
# Слова, которые встречаются только в примерах. Человек вставляет строку из
# инструкции целиком чаще, чем кажется, и получает в ответ ошибку DNS вместо
# понятного «это заглушка».
PLACEHOLDER_MARKERS = ("ваш", "your", "адрес", "ключ", "<", ">", "…", "example",
                       "xxx", "имя", "сюда")
...
def url_problem(url: str) -> str:
    ...
    if any(marker in host.lower() for marker in PLACEHOLDER_MARKERS):
